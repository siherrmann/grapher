	...
	tests := []struct {
	...
	}{
		{
			...
			envs: map[string]string{
				...
				"GRAPHER_DB_PORT":     dbPort,
				"GRAPHER_DB_DATABASE": "database",
				"GRAPHER_DB_USERNAME": "user",
		...
		},
		{
			...
			envs: map[string]string{
				...
				"GRAPHER_DB_DATABASE": "database",
				"GRAPHER_DB_USERNAME": "user",
				// "GRAPHER_DB_PASSWORD": "password", // Intentionally missing
				"GRAPHER_DB_SCHEMA": "public",
			},
			expectError: true,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectError {
				...
				assert.Equal(t, test.envs["GRAPHER_DB_HOST"], dbConfig.Host, "expected host to be 'localhost', got %s", dbConfig.Host)
				assert.Equal(t, test.envs["GRAPHER_DB_PORT"], dbConfig.Port, "expected port to be %s, got %s", dbPort, dbConfig.Port)
				assert.Equal(t, test.envs["GRAPHER_DB_DATABASE"], dbConfig.Database, "expected database to be 'grapher_test', got %s", dbConfig.Database)
				assert.Equal(t, test.envs["GRAPHER_DB_USERNAME"], dbConfig.Username, "expected username to be 'postgres', got %s", dbConfig.Username)
				assert.Equal(t, test.envs["GRAPHER_DB_PASSWORD"], dbConfig.Password, "expected password to be empty, got %s", dbConfig.Password)
				assert.Equal(t, test.envs["GRAPHER_DB_SCHEMA"], dbConfig.Schema, "expected schema to be 'public', got %s", dbConfig.Schema)
			}
